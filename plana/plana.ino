...
void loop() {
  if (gsm.available()) {
     ...
     if (phoneNumber.indexOf("+256754832171") != -1 || phoneNumber.indexOf("+256200943332") != -1 ) {
     
     if (receivedMessage.indexOf("ALL ON") != -1 || receivedMessage.indexOf("BYONA ON") != -1 || receivedMessage.indexOf("BYOSI ON") != -1 ) {
    ...
    else if (receivedMessage.indexOf("ALL OFF") != -1 || receivedMessage.indexOf("BYONA OFF") != -1 || receivedMessage.indexOf("BYOSI OFF") != -1  ) {
    ...
    if (receivedMessage.indexOf("FRONT ON") != -1  || receivedMessage.indexOf("MUMASO YAKA") != -1 || receivedMessage.indexOf("IBURANGISI ON") != -1 ) {
      digitalWrite(FRONT, HIGH); 
      sendmassage("Front Lights are on");
     
    }
    else if (receivedMessage.indexOf("FRONT OFF") != -1  || receivedMessage.indexOf("MUMASO VAKO") != -1 || receivedMessage.indexOf("IBURANGISI OFF") != -1) {
      digitalWrite(FRONT, LOW);
      sendmassage("Front Lights are off");
    
    }
    if (receivedMessage.indexOf("BACK ON") != -1  || receivedMessage.indexOf("MABEGA YAKA") != -1 || receivedMessage.indexOf("INYUMA ON") != -1) {
      digitalWrite(BACK, HIGH); 
      sendmassage("Back lights are on");
       
    }
    else if (receivedMessage.indexOf("BACK OFF") != -1  || receivedMessage.indexOf("MABEGA VAKO") != -1 || receivedMessage.indexOf("INYUMA OFF") != -1) {
      digitalWrite(BACK, LOW);
      sendmassage("Back lights are off");
    }
     if (receivedMessage.indexOf("CENTER ON") != -1  || receivedMessage.indexOf("MAKATI YAKA") != -1 || receivedMessage.indexOf("AKARI ON") != -1) {
      digitalWrite(CENTER, HIGH); 
      sendmassage("Center light is on");
       
    }
    else if (receivedMessage.indexOf("CENTER OFF") != -1 || receivedMessage.indexOf("MAKATI VAKO") != -1 || receivedMessage.indexOf("AKARI OFF") != -1) {
      digitalWrite(CENTER, LOW);
    ...
    if (receivedMessage.indexOf("APPLIANCE ON") != -1 || receivedMessage.indexOf("TEKAKO FRIGI") != -1) {
     ...
     n+=1;
      ...
      if(receivedMessage.indexOf(" ") != -1 ){
        ...
        digitalWrite(CENTER, LOW);
        delay(3000);
      }else if(receivedMessage.indexOf("/") != -1 ) {
        digitalWrite(FRONT, LOW);
        delay(5000);
        digitalWrite(BACK, LOW);
        delay(5000);
...
void sendmassage(String to_send){
   ...
   void SendMessagestart()
{
  gsm.println("AT+CMGS=\"+256754832171\"\r"); 
  delay(1000);
